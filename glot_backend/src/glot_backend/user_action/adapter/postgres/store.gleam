...
pub fn new(db: db_helpers.Db) -> user_action_store.Store {
  user_action_store.Store(
    count: fn(filter) { count_user_actions(db, filter) },
    create: fn(user_action) { create_user_action(db, user_action) },
