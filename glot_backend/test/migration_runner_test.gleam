...
pub fn parse_filename_rejects_missing_name_test() {
  assert sql_file.parse_filename("0001.sql") == Error("Invalid migration filename: 0001.sql")
}
...
pub fn validate_seed_filenames_allows_no_bootstrap_test() {
  assert sql_file.validate_seeds([
    "0001_dev_seed.sql",
  ]) == Ok([sql_file.SqlFile(version: "0001", name: "dev_seed", filename: "0001_dev_seed.sql")])
}
