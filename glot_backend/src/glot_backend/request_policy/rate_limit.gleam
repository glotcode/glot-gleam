...
) -> request_policy_config.RateLimitActor {
  case account_tier {
    option.Some(account_tier) ->
      request_policy_config.AuthenticatedActor(account_tier: account_tier)
    option.None -> request_policy_config.AnonymousActor
  }
}
...
fn lookup_rate_limits(
  ...
  action: PublicAction,
  actor: request_policy_config.RateLimitActor,
) -> List(rate_limit.RateLimit) {
  case dynamic_config.lookup_rate_limit_policy(config, action) {
    option.Some(policy) ->
      request_policy_config.select_rate_limits(policy, actor)
    option.None -> []
  }
}
