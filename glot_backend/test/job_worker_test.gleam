...
pub fn maintenance_to_running_resumes_claiming_test() {
  ...
  let assert Ok(_) =
    server_mode.start_in(server_mode_name, server_mode.Maintenance)
  ...
  process.sleep(20)
  assert snapshot(control_subject).claim_count == 0
...
pub fn claimed_job_starts_attempt_and_schedules_timeout_test() {
  ...
  let assert Ok(_) = server_mode.start_in(server_mode_name, server_mode.Running)
  let server_mode_subject = process.named_subject(server_mode_name)
...
fn test_deps(
  ...
  job_worker.Deps(
    ...
    claim_next_job: fn(_) {
      ...
      process.send(control_subject, TakeClaimJob(reply))
      Ok(process.receive_forever(reply))
    },
    process_job: fn(_, _) {
    ...
    job_started: fn() {
      process.send(control_subject, RecordJobStarted)
      Nil
    },
    job_finished: fn() {
      process.send(control_subject, RecordJobFinished)
      Nil
    },
    ...
    send_after: fn(subject, _delay, message) {
      case message {
        ...
        _ -> process.send_after(subject, 1, message)
      }
    },
...
fn wait_for_snapshot(
  ...
  let current = snapshot(control_subject)

  case predicate(current) || attempts_remaining <= 0 {
    True -> current
    False -> {
      process.sleep(10)
      wait_for_snapshot(control_subject, predicate, attempts_remaining - 1)
    }
  }
}
