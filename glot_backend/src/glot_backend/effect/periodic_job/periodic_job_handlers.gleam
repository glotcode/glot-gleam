...
pub fn new(db: pog.Connection) -> PeriodicJobHandlers {
  PeriodicJobHandlers(
    ...
    create_periodic_job: fn(periodic_job) { create_periodic_job(db, periodic_job) },
    update_periodic_job: fn(periodic_job) { update_periodic_job(db, periodic_job) },
  )
}
