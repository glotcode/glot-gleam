...
pub type User {
  User(id: BitArray, email: email.Email, created_at: Timestamp)
}

pub fn encode(user: User) -> json.Json {
  json.object([
    #("id", json.string(uuid_helpers.raw_to_string(user.id))),
...
pub fn user_decoder(is_email: regexp.Regexp) -> decode.Decoder(User) {
  use id <- decode.field("id", uuid_helpers.raw_decoder())
