...
pub fn start(db: pog.Connection) {
  actor.new_with_initialiser(1000, fn(subject) {
    let initial_state = State(subject:, db:, consecutive_failures: 0)
...
pub fn supervised(db: pog.Connection) {
...
fn handle_message(state: State, message: Message) -> actor.Next(State, Message) {
  case message {
    Tick -> {
      ...
      let _ =
        process.send_after(state.subject, health_check_interval_ms, Tick)
